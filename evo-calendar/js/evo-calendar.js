...
;(function(factory) {
            ...
            _.initials = {
                ...
                dates: {
                    en: {
                        ...
                        noEventForToday: "No event(s) for today",
                        noEventForThisDay: "No event(s) for this day",
                    ...
                    nl: {
                        ...
                        noEventForToday: "Geen event voor vandaag",
                        noEventForThisDay: "Geen event voor deze dag",
    ...
    EvoCalendar.prototype.buildTheBones = function() {
        ...
        if (!_.$elements.calendarEl.html()) {
            ...
            markup = '<div class="calendar-sidebar">'+
                        ...
                        console.log(_.$current.year);
                            markup += '<button class="icon-button" role="button" data-year-val="prev" title="'+_.initials.dates[_.options.language].previousYearText+'">'+
    ...
    function convert24hrto12hr(hr24Time) {
        var timeArray = hr24Time.split(":");
    ...
    EvoCalendar.prototype.addEventList = function(event_data) {
        ...
        if (event_data.location) { 
            if (event_data.time)
            {
                markup += '<p class="event-desc">Location: '+event_data.location+'; Time: '+ convert24hrto12hr(event_data.time) +'</p>';
            }
            else
            {
                markup += '<p class="event-desc">Location: '+event_data.location+'</p>';
            }
        }
        // if (event_data.description) markup += '<p class="event-desc">Description: '+event_data.description+'</p>';
        // if (event_data.link) markup += '<a href="'+event_data.link+'" target="_blank" class="btn-action" id="event-link-btn">Sign-up</a>';
        markup += '</div>';
        markup += '</div>';
        eventListEl.append(markup);
    ...
    EvoCalendar.prototype.buildSidebarYear = function() {
        ...
        var prevYearButton = _.$elements.sidebarEl.find('.calendar-year button[data-year-val="prev"]');
        if (_.$active.year <= new Date().getFullYear()) {
            prevYearButton.hide();
        } else {
            prevYearButton.show();
        }
    ...
    EvoCalendar.prototype.buildSidebarMonths = function() {
        ...
        monthList.find('.month').hide(); // hide all months

        var startMonth = 0;
        if (_.$active.year == new Date().getFullYear()) {
            startMonth = new Date().getMonth(); // show from current month onwards
        }
        for(var i = startMonth; i < _.$label.months.length; i++) {
            monthList.find('.month[data-month-val="'+i+'"]').show();
        }
    }
    ...
    ****************/
    ...
    EvoCalendar.prototype.selectEvent = function(event) {
        ...
        // Open target URL in a new tab or window
        if (modified_event.link) {
            window.open(modified_event.link, '_blank');
        }
